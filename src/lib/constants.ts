 ...
 * MapView의 강제 회색 처리(목적지가 없으면 무조건 회색) 때문에 목적지를
 * 넣은 뒤에만 작용해 547개 중 535개가 통근권으로 잡히며 판별력만 잃었다.
 * 그 강제 회색 처리 자체를 없애는 방안도 시도했으나(목적지 없을 때도 전
 * 지역을 등급색으로 보여줌) 실사용해보니 불필요하다고 판단해 되돌렸다 —
 * 목적지가 없으면 지금도 전 지역이 회색이다(MapView.tsx의 hasDestination
 * 게이트). 이 되돌림과 무관하게, 통근 기본값은 원래 판별력 있던 40분으로
 * 되돌린다 — 90분이 판별력을 잃게 만든 문제는 회색 처리 여부와 별개다.
 */
...
/**
 * 통근시간 페널티 Φ(t) — 종합 점수에 곱하는 비보상적(non-compensatory) 감쇠.
 *
 * 통근시간은 원래 필터(슬라이더 상한)로만 쓰였고 순위엔 전혀 안 들어갔다.
 * 그래서 문정역 목적지 기준으로 통근 57분짜리 공릉1동이 종합점수만으로
 * 통근 16.7분짜리 장지동보다 위에 오는 일이 생겼다 — 치안·가격·편의가 아무리
 * 좋아도 통근이 나쁘면 그 조합을 "괜찮은 선택"으로 추천하면 안 된다는 게
 * 이 페널티의 취지다. 가중합(보상적)이 아니라 곱셈(비보상적)을 쓰는 이유가
 * 그것이다 — 다른 축이 아무리 높아도 통근이 나쁘면 점수가 따라 죽는다.
 *
...
export const COMMUTE_DECAY_POWER = 2.8;
