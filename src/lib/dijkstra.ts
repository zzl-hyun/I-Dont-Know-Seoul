...
export interface ShortestPaths {
   ...
   * 화면 표시용 가중치 없는 모델 시간은 `real`을 써야 한다.
   */
  dist: Float64Array;
  /**
   * nodeId → 화면 표시용 모델 소요시간(분). 엣지 비용은 가중치가 붙지 않으므로,
   ...
   * 547개 동의 경로를 미리 만들지 않고 이 배열만 들고 있다가, 사용자가 고른
   * 동 하나를 그때 되짚는다.
   */
  prev: Int32Array;
}
...
export interface Seed {
  ...
  cost: number;
  /**
   * 화면에 표시할 가중치 없는 초기 비용(분). 생략하면 `cost`와 같다 — 도보
